...
def main():
    print("🚀 AI Watchlist Runner - Autonomous Mode")
    ...
    if not watchlist:
        ...
        print("⚠️ No items in watchlist.json. Please add some!")
        return
    ...
    for i, item in enumerate(watchlist):
        ...
        if not WatchlistService.should_run_analysis(item):
            print(f"⏭️  Skipping {ticker} (Last run: {item.get('last_run')}). Next in {(item.get('frequency_days', 30) - (datetime.now() - datetime.strptime(item.get('last_run', '1970-01-01'), '%Y-%m-%d')).days)} days.")
            continue

        print(f"\n🔄 Analyzing due item: {ticker} ({name})")
        ...
        if last_action != new_action:
            print(f"⚠️  CHANGE DETECTED: {last_action} -> {new_action}")
            item["action_date"] = datetime.now().strftime("%Y-%m-%d")
        else:
            print(f"✅ Status unchanged: {new_action}")
            # Ensure action_date exists even if no change
            if "action_date" not in item:
                item["action_date"] = item.get("last_run", "1970-01-01")

        # Save Analysis to File
        report_text = f"**Date:** {datetime.now().strftime('%Y-%m-%d')}\n**Action:** {analysis.action}\n**Reason:** {analysis.long_reason}\n\n*Generated by AI Watchlist Runner*"
        saved_path = StorageService.save_analysis(ticker, report_text, suffix="-WATCH")
        ...
        print(f"💾 Watchlist updated for {ticker}")
    ...
    if not updates_made:
        print("\n✅ All items are up to date. No analysis needed today.")
    else:
        print("\n✅ Batch complete.")
